...
namespace Argix.Finance {
    ...
    public partial class ThisWorkbook {
        //Members
        private const string USP_INVOICE = "uspInvPartyCityInvoiceByReleaseDateGet",TBL_INVOICE = "ClientInvoiceTable";
        ...
        private void ThisWorkbook_Startup(object sender,System.EventArgs e) {
            ...
            try {
                ...
                string clid="",invoice="";  
#if DEBUG
                clid = "096"; invoice = "330908400";    //Amscan: 048, 330948100  Party City: 096, 330908400
#endif
                if (cmd != null) {
        ...
        private void createDetailTotals(InvoiceDataset.ClientInvoiceTableDataTable invoiceTable) {
            ...
            ws.Range[ws.Cells[ROW0_DETAIL + rowCount + 1,5],ws.Cells[ROW0_DETAIL + rowCount + 1,5]].NumberFormat = "#,###_);(#,###)";
            ws.Range[ws.Cells[ROW0_DETAIL + rowCount + 1,5],ws.Cells[ROW0_DETAIL + rowCount + 1,5]].HorizontalAlignment = Excel.XlHAlign.xlHAlignRight;
            ws.Range[ws.Cells[ROW0_DETAIL + rowCount + 1,9],ws.Cells[ROW0_DETAIL + rowCount + 1,9]].NumberFormat = "#,##0_);(#,##0)";
            ws.Range[ws.Cells[ROW0_DETAIL + rowCount + 1,9],ws.Cells[ROW0_DETAIL + rowCount + 1,9]].HorizontalAlignment = Excel.XlHAlign.xlHAlignRight;
            ws.Range[ws.Cells[ROW0_DETAIL + rowCount + 1,15],ws.Cells[ROW0_DETAIL + rowCount + 1,15]].NumberFormat = "$#,##0.00_);($#,##0.00)";
            ws.Range[ws.Cells[ROW0_DETAIL + rowCount + 1,15],ws.Cells[ROW0_DETAIL + rowCount + 1,15]].HorizontalAlignment = Excel.XlHAlign.xlHAlignRight;
            ...
            //Footer
            ws.Range[ws.Cells[ROW0_DETAIL + rowCount + 3,1],ws.Cells[ROW0_DETAIL + rowCount + 3,17]].Value2 = new object[1,17] { { "PLEASE REFERENCE INVOICE# " + invoiceTable[0].InvoiceNumber + " WHEN REMITTING PAYMENT I.C.C. REGULATIONS REQUIRE THAT THIS BILL BE PAID WITHIN " + invoiceTable[0].PaymentDays + " DAYS","","","","","","","","","","","","","","","","" } };
            ws.Range[ws.Cells[ROW0_DETAIL + rowCount + 4,1],ws.Cells[ROW0_DETAIL + rowCount + 4,17]].Value2 = new object[1,17] { { "","","A SERVICE CHARGE OF 1.5% PER MONTH IS ADDED TO ALL PAST DUE INVOICES","","","","","","","","","","","","","","" } };
            ws.Range[ws.Cells[ROW0_DETAIL + rowCount + 5,1],ws.Cells[ROW0_DETAIL + rowCount + 5,17]].Value2 = new object[1,17] { { "","","REMIT TO: " + invoiceTable[0].RemitToName.Trim() + " " + invoiceTable[0].RemitToAddressLine1.Trim() + " " + invoiceTable[0].RemitToAddressLine2.Trim() + " " + invoiceTable[0].RemitToCity.Trim() + ", " + invoiceTable[0].RemitToState + " " + invoiceTable[0].RemitToZip + "-" + invoiceTable[0].RemitToZip4,"","","","","","","","","","","","","","" } };
        }

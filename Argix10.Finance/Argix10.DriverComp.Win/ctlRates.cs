...
//                          and OnGridBeforeRowUpdate() to use new DriverCompDataset fields
//                          MaximumAmount, MaximumTriggerField, & MaximumTriggerValue.
//          07/17/09 (jph)- updated grids to reflect revised fields types in DriverCompDataset;
    ...
    public partial class ctlRates :UserControl {
            ...
            set {
                ...
                try {
                    ...
                    if(this.mRates != null) {
                        ...
                        this.grdMileageRates.DataSource = this.grdUnitRates.DataSource = this.grdMileageRouteRates.DataSource = this.grdUnitRouteRates.DataSource = new DriverCompDataset();
                    }
        ...
        private void OnRateTypeSelected(object sender,System.EventArgs e) {
            ...
            try {
                ...
                UltraGrid grid = null;
                switch(this.cboRateType.SelectedItem.ToString()) {
                    case "Vehicle Mileage Rates":   grid = this.grdMileageRates; break;
                    case "Vehicle Unit Rates":      grid = this.grdUnitRates; break;
                    case "Route Mileage Rates":     grid = this.grdMileageRouteRates; break;
                    case "Route Unit Rates":        grid = this.grdUnitRouteRates; break;
                }
                grid.Visible = true;
        ...
        private void OnGridInitializeRow(object sender, Infragistics.Win.UltraWinGrid.InitializeRowEventArgs e) {
            //
            try {
                bool canEdit = ((this.mRates != null) && RoleServiceGateway.IsBillingSupervisor && (e.Row.IsAddRow || (this.mRates.EffectiveDate.CompareTo(Convert.ToDateTime(e.Row.Cells["EffectiveDate"].Value)) == 0)));
                    ...
                    case "RateMileageTable":
                        if(e.Cell.Row.IsAddRow) {
                            if(e.Cell.Row.Cells["AgentNumber"].Value.ToString() == "") {
                                ...
                                e.Cell.Row.Cells["EffectiveDate"].Value = this.mRates.EffectiveDate;

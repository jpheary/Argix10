...
namespace Argix.Freight {
    //
    public partial class winInbound:Form,ISchedule {
        //Members
        private UltraGridSvc mGridSvc = null, mGridTSvc=null;
        private bool mIsDragging = false;
        private InboundAutoRefreshService mAutoRefreshSvc = null;
        ...
        public winInbound() {
            ...
            try {
                ...
                this.mAutoRefreshSvc = new InboundAutoRefreshService(this);
            }
        ...
        public void Cancel() {
            if (this.csCancel.Enabled) {
                ...
                if (MessageBox.Show(this,"Are you sure you want to cancel trip " + id.ToString() + "?",App.Product,MessageBoxButtons.YesNo,MessageBoxIcon.Question) == System.Windows.Forms.DialogResult.Yes) {
                    FreightGateway.CancelScheduledInboundFreight(id,DateTime.Now,Environment.UserName);
                    Refresh();
                }
            }
        }
        ...
        public void Print(bool showDialog) {
            ...
            try {
                ...
                string caption = "Inbound Schedule ";
                switch (this.cboSchedule.SelectedItem.ToString()) {
                    case "Today": caption = "Inbound Schedule " + DateTime.Today.ToString("MM/dd/yyyy"); break;
                    case "Yesterday": caption = "Inbound Schedule " + DateTime.Today.AddDays(-1).ToString("MM/dd/yyyy"); ; break;
                    case "Advanced": caption = "Inbound Schedule Advanced"; break;
                    case "Archive": caption = "Inbound Schedule Archive"; break;
                }
        ...
        public override void Refresh() {
            try {
                this.Cursor = Cursors.WaitCursor;
                reportStatus(new StatusEventArgs("Refreshing Inbound Schedule..."));
                this.mGridSvc.CaptureState();
                this.mSchedule.Clear();
                switch (this.cboSchedule.SelectedItem.ToString()) {
                    case "Today": this.mSchedule.Merge(FreightGateway.ViewInboundSchedule()); break;
                    case "Yesterday": this.mSchedule.Merge(FreightGateway.ViewInboundScheduleYesterday()); break;
                    case "Advanced": this.mSchedule.Merge(FreightGateway.ViewInboundScheduleAdvanced()); break;
                    case "Archive": this.mSchedule.Merge(FreightGateway.ViewInboundScheduleArchive()); break;
                }
                this.mGridSvc.RestoreState();

                this.mGridTSvc.CaptureState();
                this.mTemplates.Clear();
                this.mTemplates.Merge(FreightGateway.ViewInboundScheduleTemplates());
        ...
        private void OnFormLoad(object sender,EventArgs e) {
            ...
            try {
                ...
                #endregion
                this.ColumnHeaders = global::Argix.Properties.Settings.Default.InboundColumnHeaders;
        ...
        private void OnFormActivated(object sender,EventArgs e) {
            ...
            try {
                if(this.cboSchedule.SelectedItem.ToString() == "Today") {
                    reportStatus(new StatusEventArgs("Starting auto-refresh on Inbound Schedule..."));
                    this.mAutoRefreshSvc.Start();
                }
                else {
                    reportStatus(new StatusEventArgs("Stopping auto-refresh on Inbound Schedule..."));
        ...
        private void OnFormClosing(object sender,FormClosingEventArgs e) {
            ...
            try {
                if (!e.Cancel) {
                    //Save settings
                    global::Argix.Properties.Settings.Default.InboundColumnHeaders = this.ColumnHeaders;
        ...
        private void OnScheduleAfterRowUpdate(object sender,Infragistics.Win.UltraWinGrid.RowEventArgs e) {
            ...
            try {
                ...
                DispatchDataset.InboundScheduleTableRow trip = (DispatchDataset.InboundScheduleTableRow)this.mSchedule.InboundScheduleTable.Select("ID=" + id)[0];
                FreightGateway.ChangeScheduledInboundFreight(trip);
                Refresh();
            }
            catch (Exception ex) { App.ReportError(ex,true,LogLevel.Warning); }
        }
        ...
        private void OnDragDrop(object sender,System.Windows.Forms.DragEventArgs e) {
            ...
            try {
                ...
                if (oData.GetDataPresent(DataFormats.StringFormat,true)) {
                    for (int i = 0;i < this.mTemplates.InboundScheduleTable.Rows.Count;i++) {
                        if (this.mTemplates.InboundScheduleTable[i].Selected == true) {
                            DispatchDataset.InboundScheduleTableRow trip = new DispatchDataset().InboundScheduleTable.NewInboundScheduleTableRow();
                            trip.ItemArray = this.mTemplates.InboundScheduleTable[i].ItemArray;
                            trip.CreateUserID = Environment.UserName;
                            trip.Created = DateTime.Now;
                            switch (this.cboSchedule.SelectedItem.ToString()) {
                                case "Today": trip.ScheduleDate = DateTime.Today; break;
        ...
        private void OnItemClick(object sender,System.EventArgs e) {
            dlgInboundFreight dlg = null;
            DispatchDataset.InboundScheduleTableRow trip = null;
            int id = 0;
            try {
                ToolStripItem item = (ToolStripItem)sender;
                switch (item.Name) {
                    case "csNew":
                        trip = new DispatchDataset().InboundScheduleTable.NewInboundScheduleTableRow();
                        switch (this.cboSchedule.SelectedItem.ToString()) {
                            case "Today": trip.ScheduleDate = DateTime.Today; break;
                            case "Advanced": trip.ScheduleDate = DateTime.Today.AddDays(1); break;
                        }
                        ...
                        trip.IsTemplate = false;
                        dlg = new dlgInboundFreight(trip);
                        dlg.Font = this.Font;
                    ...
                    case "csOpen":
                        ...
                        DispatchDataset.InboundScheduleTableRow _freight = (DispatchDataset.InboundScheduleTableRow)this.mSchedule.InboundScheduleTable.Select("ID=" + id)[0];
                        trip = new DispatchDataset().InboundScheduleTable.NewInboundScheduleTableRow();
                    ...
                    case "csClone":
                        id = Convert.ToInt32(this.grdSchedule.ActiveRow.Cells["ID"].Value);
                        trip = (DispatchDataset.InboundScheduleTableRow)this.mSchedule.InboundScheduleTable.Select("ID=" + id)[0];
                        DispatchDataset.InboundScheduleTableRow clone = new DispatchDataset().InboundScheduleTable.NewInboundScheduleTableRow();
                        switch (this.cboSchedule.SelectedItem.ToString()) {
                        ...
                        dlg = new dlgInboundFreight(clone);
                        dlg.Font = this.Font;
                        if (dlg.ShowDialog(this) == DialogResult.OK) {
                            FreightGateway.AddScheduledInboundFreight(clone);
                            Refresh();
                        }
                        break;
                    case "csCancel":
                    ...
                    case "csTempNew":
                        ...
                        dlg = new dlgInboundFreight(trip,true);
                        dlg.Font = this.Font;
                        if (dlg.ShowDialog(this) == DialogResult.OK) {
                            FreightGateway.AddScheduledInboundFreight(trip);
                            Refresh();
                    ...
                    case "csTempOpen":
                        ...
                        DispatchDataset.InboundScheduleTableRow _template = (DispatchDataset.InboundScheduleTableRow)this.mTemplates.InboundScheduleTable.Select("ID=" + id)[0];
                        trip = new DispatchDataset().InboundScheduleTable.NewInboundScheduleTableRow();
        ...
        private void OnTemplateSelected(object sender,Infragistics.Win.UltraWinGrid.AfterSelectChangeEventArgs e) {
            ...
            try {
                ...
                for (int i = 0;i < this.mTemplates.InboundScheduleTable.Rows.Count;i++) this.mTemplates.InboundScheduleTable[i].Selected = false;
                ...
                for (int i = 0;i < this.grdTemplates.Selected.Rows.Count;i++) {
                    ...
                    for (int j = 0;j < this.mTemplates.InboundScheduleTable.Rows.Count;j++) {
                        if (this.mTemplates.InboundScheduleTable[j].ID == templateID) {
                            this.mTemplates.InboundScheduleTable[j].Selected = true;
                            break;
                        }
                    }
                }
    ...
    public class InboundAutoRefreshService {
        //Members
        private System.Windows.Forms.Timer mTimer = null;
        private BackgroundWorker mWorker = null;

        //Interface
        public InboundAutoRefreshService(winInbound postback) {
            //
        ...
        private void OnAutoRefresh(object sender,DoWorkEventArgs e) {
            ...
            try { e.Result = FreightGateway.ViewInboundSchedule(); }
            catch { }
        }
    }
}

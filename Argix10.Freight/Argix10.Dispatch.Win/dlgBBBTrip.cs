...
namespace Argix.Freight {
	//
    public class dlgBBBTrip:System.Windows.Forms.Form {
		//Members
        private DispatchDataset.BBBScheduleTableRow mTrip = null;
        ...
        public dlgBBBTrip(DispatchDataset.BBBScheduleTableRow trip,bool isTemplate = false) {
			//Constructor
			try {
				InitializeComponent();
                this.mTrip = trip;
                if(this.mTrip.IsIDNull()) this.mTrip.ID = 0;
        ...
        private void OnValidateForm(object sender,EventArgs e) {
			...
			try {
                ...
                this.chkConfirmed.Enabled = !this.mIsTemplate && this.mTrip.ID > 0;
                this.dtpScheduledDeparture.Enabled = true;
                this.dtpActualDeparture.Enabled = !this.mIsTemplate && this.mTrip.ID > 0;
                this.dtpScheduledArrival.Enabled = true;
                this.dtpActualArrival.Enabled = !this.mIsTemplate && this.mTrip.ID > 0;
                this.txtComments.Enabled = true;
                this.txtTDSNumber.Enabled = !this.mIsTemplate && this.mTrip.ID > 0;
            ...
            try {
                ...
                switch(btn.Name) {
                    ...
                    case "btnOk":
                        ...
                        this.mTrip.IsLiveUnload = this.chkIsLiveUnload.Checked;
                        this.mTrip.Amount = int.Parse(this.txtAmount.Text);
                        this.mTrip.AmountType = this.cboAmountType.Text;
                        this.mTrip.FreightType = this.cboFreightType.Text;
                        if(this.txtTDSNumber.Text.Trim().Length > 0) this.mTrip.TDSNumber = this.txtTDSNumber.Text;

...
namespace Argix.Freight {
    //
    public partial class winBBB:Form,ISchedule {
        //Members
        private UltraGridSvc mGridSvc = null, mGridTSvc=null;
        private bool mIsDragging = false;
        private BBBAutoRefreshService mAutoRefreshSvc = null;
        ...
        public winBBB() {
            ...
            try {
                ...
                this.mAutoRefreshSvc = new BBBAutoRefreshService(this);
            }
        ...
        public void Cancel() {
            if (this.csCancel.Enabled) {
                ...
                if (MessageBox.Show(this,"Are you sure you want to cancel trip " + id.ToString() + "?",App.Product,MessageBoxButtons.YesNo,MessageBoxIcon.Question) == System.Windows.Forms.DialogResult.Yes) {
                    FreightGateway.CancelBBBScheduleTrip(id,DateTime.Now,Environment.UserName);
                    Refresh();
                }
            }
        }
        public bool CanSave { get { return (this.grdSchedule.Rows != null ? this.grdSchedule.Rows.Count > 0 : false); } }
        public void Save(string filename) { new Argix.ExcelFormat().Transform(this.mSchedule,"BBBScheduleTable",filename); }
        ...
        public void Print(bool showDialog) {
            ...
            try {
                ...
                string caption = "BBB Schedule ";
                switch (this.cboSchedule.SelectedItem.ToString()) {
                    case "Today": caption = "BBB Schedule " + DateTime.Today.ToString("MM/dd/yyyy"); break;
                    case "Yesterday": caption = "BBB Schedule " + DateTime.Today.AddDays(-1).ToString("MM/dd/yyyy"); ; break;
                    case "Advanced": caption = "BBB Schedule Advanced"; break;
                    case "Archive": caption = "BBB Schedule Archive"; break;
                }
        ...
        public override void Refresh() {
            try {
                this.Cursor = Cursors.WaitCursor;
                reportStatus(new StatusEventArgs("Refreshing BBB Schedule..."));
                this.mGridSvc.CaptureState();
                this.mSchedule.Clear();
                switch (this.cboSchedule.SelectedItem.ToString()) {
                    case "Today": this.mSchedule.Merge(FreightGateway.ViewBBBSchedule()); break;
                    case "Yesterday": this.mSchedule.Merge(FreightGateway.ViewBBBScheduleYesterday()); break;
                    case "Advanced": this.mSchedule.Merge(FreightGateway.ViewBBBScheduleAdvanced()); break;
                    case "Archive": this.mSchedule.Merge(FreightGateway.ViewBBBScheduleArchive()); break;
                }
                this.mGridSvc.RestoreState();

                //this.mGridTSvc.CaptureState();
                //this.mTemplates.Clear();
                //this.mTemplates.Merge(FreightGateway.ViewBBBScheduleTemplates());
        ...
        private void OnFormLoad(object sender,EventArgs e) {
            ...
            try {
                ...
                #endregion
                this.ColumnHeaders = global::Argix.Properties.Settings.Default.BBBColumnHeaders;
        ...
        private void OnFormActivated(object sender,EventArgs e) {
            ...
            try {
                if(this.cboSchedule.SelectedItem.ToString() == "Today") {
                    reportStatus(new StatusEventArgs("Starting auto-refresh on BBB Schedule..."));
                    this.mAutoRefreshSvc.Start();
                }
                else {
                    reportStatus(new StatusEventArgs("Stopping auto-refresh on BBB Schedule..."));
        ...
        private void OnFormClosing(object sender,FormClosingEventArgs e) {
            ...
            try {
                if (!e.Cancel) {
                    //Save settings
                    global::Argix.Properties.Settings.Default.BBBColumnHeaders = this.ColumnHeaders;
        ...
        private void OnScheduleInitializeRow(object sender,InitializeRowEventArgs e) {
            ...
            try {
                if (this.cboSchedule.SelectedItem.ToString() == "Today" && (RoleServiceGateway.IsDispatchSupervisor || RoleServiceGateway.IsDispatchClerk)) {
                    ...
                    //if(e.Row.Cells["FreightType"].Value.ToString().ToLower() == "pcs" && e.Row.Cells["Cancelled"].Value.ToString().Trim().Length == 0) {
                    //    //
                    //    DateTime created = DateTime.Parse(e.Row.Cells["Created"].Value.ToString());
                    //    DateTime actualArrival = e.Row.Cells["ActualArrival"].Value == DBNull.Value ? DateTime.MinValue : DateTime.Parse(e.Row.Cells["ActualArrival"].Value.ToString());
                    //    DateTime cancelled = e.Row.Cells["Cancelled"].Value == DBNull.Value ? DateTime.MinValue : DateTime.Parse(e.Row.Cells["Cancelled"].Value.ToString());
                    //    if(cancelled.CompareTo(DateTime.MinValue) == 0 && (actualArrival.CompareTo(DateTime.MinValue) == 0 ? DateTime.Now.AddHours(-48).CompareTo(created) > 0 : actualArrival.AddHours(-48).CompareTo(created) > 0)) e.Row.Cells["ActualArrival"].Appearance.BackColor = Color.Red;
                    //}
                }
            ...
            try {
                if (e.KeyCode == Keys.Enter && this.grdSchedule.ActiveRow != null) {
                    ...
                    if (this.grdSchedule.ActiveRow != null) {
                        int id = int.Parse(this.grdSchedule.ActiveRow.Cells["ID"].Value.ToString());
                        DataRow[] trips = this.mSchedule.BBBScheduleTable.Select("ID=" + id);
                    ...
                    if (this.cboSchedule.SelectedItem.ToString() == "Today" || this.cboSchedule.SelectedItem.ToString() == "Advanced") {
                        ...
                        if (o != null) {
                            ...
                            FreightGateway.AddBBBScheduleTrip(trip);
                            Refresh();
                        }
                    }
                }
        ...
        private void OnScheduleAfterRowUpdate(object sender,Infragistics.Win.UltraWinGrid.RowEventArgs e) {
            ...
            try {
                ...
                DispatchDataset.BBBScheduleTableRow trip = (DispatchDataset.BBBScheduleTableRow)this.mSchedule.BBBScheduleTable.Select("ID=" + id)[0];
                FreightGateway.ChangeBBBScheduleTrip(trip);
                Refresh();
            }
            catch (Exception ex) { App.ReportError(ex,true,LogLevel.Warning); }
        }
        ...
        private void OnDragDrop(object sender,System.Windows.Forms.DragEventArgs e) {
            ...
            try {
                ...
                if (oData.GetDataPresent(DataFormats.StringFormat,true)) {
                    for (int i = 0;i < this.mTemplates.BBBScheduleTable.Rows.Count;i++) {
                        if (this.mTemplates.BBBScheduleTable[i].Selected == true) {
                            DispatchDataset.BBBScheduleTableRow trip = new DispatchDataset().BBBScheduleTable.NewBBBScheduleTableRow();
                            trip.ItemArray = this.mTemplates.BBBScheduleTable[i].ItemArray;
                            trip.CreateUserID = Environment.UserName;
                            trip.Created = DateTime.Now;
                            switch (this.cboSchedule.SelectedItem.ToString()) {
                                case "Today": trip.ScheduleDate = DateTime.Today; break;
        ...
        private void OnItemClick(object sender,System.EventArgs e) {
            dlgBBBTrip dlg = null;
            DispatchDataset.BBBScheduleTableRow trip = null;
            int id = 0;
            try {
                ToolStripItem item = (ToolStripItem)sender;
                switch (item.Name) {
                    case "csNew":
                        trip = new DispatchDataset().BBBScheduleTable.NewBBBScheduleTableRow();
                        switch (this.cboSchedule.SelectedItem.ToString()) {
                            case "Today": trip.ScheduleDate = DateTime.Today; break;
                            case "Advanced": trip.ScheduleDate = DateTime.Today.AddDays(1); break;
                        }
                    ...
                    case "csOpen":
                        ...
                        DispatchDataset.BBBScheduleTableRow _freight = (DispatchDataset.BBBScheduleTableRow)this.mSchedule.BBBScheduleTable.Select("ID=" + id)[0];
                        trip = new DispatchDataset().BBBScheduleTable.NewBBBScheduleTableRow();
                    ...
                    case "csClone":
                        id = Convert.ToInt32(this.grdSchedule.ActiveRow.Cells["ID"].Value);
                        trip = (DispatchDataset.BBBScheduleTableRow)this.mSchedule.BBBScheduleTable.Select("ID=" + id)[0];
                        DispatchDataset.BBBScheduleTableRow clone = new DispatchDataset().BBBScheduleTable.NewBBBScheduleTableRow();
                        switch (this.cboSchedule.SelectedItem.ToString()) {
                        ...
                        dlg = new dlgBBBTrip(clone);
                        dlg.Font = this.Font;
                        if (dlg.ShowDialog(this) == DialogResult.OK) {
                            FreightGateway.AddBBBScheduleTrip(clone);
                            Refresh();
                        }
                        break;
                    case "csCancel":
                    ...
                    case "csTempOpen":
                        ...
                        DispatchDataset.BBBScheduleTableRow _template = (DispatchDataset.BBBScheduleTableRow)this.mTemplates.BBBScheduleTable.Select("ID=" + id)[0];
                        trip = new DispatchDataset().BBBScheduleTable.NewBBBScheduleTableRow();
        ...
        private void OnTemplateSelected(object sender,Infragistics.Win.UltraWinGrid.AfterSelectChangeEventArgs e) {
            ...
            try {
                ...
                for (int i = 0;i < this.mTemplates.BBBScheduleTable.Rows.Count;i++) this.mTemplates.BBBScheduleTable[i].Selected = false;
                ...
                for (int i = 0;i < this.grdTemplates.Selected.Rows.Count;i++) {
                    ...
                    for (int j = 0;j < this.mTemplates.BBBScheduleTable.Rows.Count;j++) {
                        if (this.mTemplates.BBBScheduleTable[j].ID == templateID) {
                            this.mTemplates.BBBScheduleTable[j].Selected = true;
                            break;
                        }
                    }
                }
    ...
    public class BBBAutoRefreshService {
        ...
        private void OnAutoRefresh(object sender,DoWorkEventArgs e) {
            ...
            try { e.Result = FreightGateway.ViewBBBSchedule(); }
            catch { }
        }
    }
}

    ...
    public class AppService:IAppService {
        ...
        public ServiceInfo GetServiceInfo() {
            ...
            try {
                ...
                if(ds != null && ds.Tables[TBL_LOCALTERMINAL] != null && ds.Tables[TBL_LOCALTERMINAL].Rows.Count > 0) {
                    ...
                    string description = "";
                    try {
                        DataSet _ds = new DataService().FillDataset(this.mConnectionID, "SELECT @@Servername AS ServerName", new string[] { TBL_LOCALTERMINAL });
                        if(_ds != null && _ds.Tables[TBL_LOCALTERMINAL] != null && _ds.Tables[TBL_LOCALTERMINAL].Rows.Count > 0)
                            description = _ds.Tables[TBL_LOCALTERMINAL].Rows[0]["ServerName"].ToString();
                    }
                    catch { }
        ...
        public void WriteLogEntry(TraceMessage m) {
            ...
            try {
                if (m.LogLevel >= this.mLogLevelFloor) {
                    ...
                    new SMTPGateway().SendITNotification(m.Source, m.User, new ApplicationException(m.Message));
                }
            }
            catch { }
        }

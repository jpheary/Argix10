...
namespace Argix.Security {
	...
	public class RoleServiceGateway {
        ...
        private const string ROLE_SHIPPINGSUPERVISOR = "Shipping Supervisor";
        private const string ROLE_SHIPPINGCLERK = "Shipping Clerk";
        private const string ROLE_WAREHOUSESUPERVISOR = "Warehouse Supervisor";
        ...
        public static string GetRoleForCurrentUser() {
            if (IsWarehouseSupervisor) return ROLE_WAREHOUSESUPERVISOR;
            else if (IsShippingSupervisor) return ROLE_SHIPPINGSUPERVISOR;
            else if (IsShippingClerk) return ROLE_SHIPPINGCLERK;
            else {
        ...
        public static bool IsShippingSupervisor {
            get {
                bool authorized = false;
                string[] roles = GetRolesForCurrentUser();
                for (int i = 0;i < roles.Length;i++) {
                    authorized = (roles[i] == ROLE_SHIPPINGSUPERVISOR);
                    if (authorized) break;
                }
                return authorized;
            }
        }
        public static bool IsShippingClerk {
            get {
                bool authorized = false;
                string[] roles = GetRolesForCurrentUser();
                for (int i = 0;i < roles.Length;i++) {
                    authorized = (roles[i] == ROLE_SHIPPINGCLERK);
                    if (authorized) break;
                }
                return authorized;
            }
        }
        public static bool IsWarehouseSupervisor {
            get {
                bool authorized = false;
                string[] roles = GetRolesForCurrentUser();
                for (int i = 0;i < roles.Length;i++) {
                    authorized = (roles[i] == ROLE_WAREHOUSESUPERVISOR);
                    if (authorized) break;
                }
                return authorized;
            }

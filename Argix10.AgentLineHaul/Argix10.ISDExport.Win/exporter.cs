...
namespace Argix.AgentLineHaul {
    ...
    public class RDS3Exporter :Exporter {
        ...
        private string formatSortedItemRecord(ISDExportDataset.SortedItemTableRow sortedItem) {
    ...
    public class RDS4Exporter:Exporter {
        ...
        public override bool Export(string exportFile,ISDClient isdClient,Pickup pickup,ISDExportDataset sortedItems) {
            ...
            try {
                ...
                writer = new StreamWriter(new FileStream(exportFile,FileMode.Create,FileAccess.ReadWrite));
                writer.BaseStream.Seek(0,SeekOrigin.Begin);
                writer.WriteLine(formatTrailerRecord(isdClient.Client,pickup));
                for(int j = 0;j < sortedItems.SortedItemTable.Rows.Count;j++)
                    writer.WriteLine(formatSortedItemRecord(isdClient.Client,(ISDExportDataset.SortedItemTableRow)sortedItems.SortedItemTable.Rows[j]));
                writer.Flush();
                exported = true;
            }
            catch(ApplicationException ex) { throw ex; }
        ...
        private string formatSortedItemRecord(string clientCode,ISDExportDataset.SortedItemTableRow sortedItem) {
    ...
    public class PCSExporter :Exporter {
        ...
        public override bool Export(string exportFile,ISDClient isdClient,Pickup pickup,ISDExportDataset sortedItems) {
            ...
            try {
                ...
                writer = new StreamWriter(new FileStream(exportFile,FileMode.Create,FileAccess.ReadWrite));
                writer.BaseStream.Seek(0,SeekOrigin.Begin);
                writer.WriteLine(formatTrailerRecord(isdClient.Client,pickup,isdClient.Scanner,isdClient.UserID));
                for(int j = 0;j < sortedItems.SortedItemTable.Rows.Count;j++)
                    writer.WriteLine(formatSortedItemRecord(isdClient.Client,(ISDExportDataset.SortedItemTableRow)sortedItems.SortedItemTable.Rows[j]));
                writer.Flush();
                exported = true;

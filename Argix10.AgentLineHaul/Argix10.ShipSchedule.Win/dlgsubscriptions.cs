	...
	public class dlgSubscriptions : System.Windows.Forms.Form {
        ...
        private void refresh() {
            ...
            try {
                ...
                rs.Url = global::Argix.Properties.Settings.Default.SQLReportsWS;
                    ...
                    for(int i=0;i<subscriptions.Length;i++) {
                        ...
                        if (paramValues != null && sub.Path == global::Argix.Properties.Settings.Default.CarrierReportPath) {
                            ...
                            if(terminalID != null && terminalID == this.mSchedule.SortCenterID.ToString() && carrierID != null && carrierID.Length > 0) {
                                ...
                                ShipScheduleDataset.ShipScheduleViewTableRow[] rows = (ShipScheduleDataset.ShipScheduleViewTableRow[])this.mSchedule.Trips.ShipScheduleViewTable.Select("carrierID=" + carrierID);
                            ...
                            if(agentNumber != null && agentNumber.Length > 0) {
                                bool openExists=false;
                                ShipScheduleDataset.ShipScheduleViewTableRow[] rows = (ShipScheduleDataset.ShipScheduleViewTableRow[])this.mSchedule.Trips.ShipScheduleViewTable.Select("AgentNumber='" + agentNumber + "' OR S2AgentNumber='" + agentNumber + "'");
                                for(int k=0;k<rows.Length;k++) { if(rows[k].IsCanceledNull()) openExists = true; }
                ...
                if(subscriptions != null) {
                    ...
                    for(int i=0;i<subscriptions.Length;i++) {
                        ...
                        if (paramValues != null && sub.Path == global::Argix.Properties.Settings.Default.CarrierReportPath) {
                            ...
                            string terminalID = getParamValue(paramValues,PARAM_TERMINALID);
                            string carrierID = getParamValue(paramValues,PARAM_CARRIERID);
                            if(terminalID == this.mSchedule.SortCenterID.ToString() && carrierID != null && this.mSchedule.Trips.ShipScheduleViewTable.Select("carrierID=" + carrierID).Length > 0) {
                                //Determine if the subscription is still checked for send (no user override)
                                SubscriptionDataset.SubscriptionTableRow[] rows = (SubscriptionDataset.SubscriptionTableRow[])this.mSubscriptionDS.SubscriptionTable.Select("SubscriptionID='" + sub.SubscriptionID + "'");
                            ...
                            string agentNumber = getParamValue(paramValues,PARAM_AGENTID);
                            if((agentNumber != null) && (this.mSchedule.Trips.ShipScheduleViewTable.Select("AgentNumber=" + agentNumber).Length > 0 || this.mSchedule.Trips.ShipScheduleViewTable.Select("S2AgentNumber=" + agentNumber).Length > 0)) {
                                //Determine if the subscription is still checked for send (no user override)
                                SubscriptionDataset.SubscriptionTableRow[] rows = (SubscriptionDataset.SubscriptionTableRow[])this.mSubscriptionDS.SubscriptionTable.Select("SubscriptionID='" + sub.SubscriptionID + "'");
                                if(rows.Length > 0 && rows[0].Send == true) {
                                    //Set subject to reflect Sort Center and schedule date; update subscription to run on a snapshot update

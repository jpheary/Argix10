...
var iUSPhone = "This field must be a valid phone number. Please reenter it now."
...
//	checks to see if any characters in a string
//	are non-safe to the file system
//	added 2/14/02 - amh
function isFileSystemSafe (s) {
	for (i = 0; i < s.length; i++)
	{
		for (j = 0; j < nonFileSystemChars.length; j++)
		{
			if (s.charAt(i) == nonFileSystemChars.charAt(j)) return false;
		}
	}
	return true;
}
...
function confirmDelete(myForm, myFormField, strMessage) {
 ...
 function changeStatePullDown() {
	var myForm = document.forms["frmName"];
	var intSelectedStateIndex = myForm.StateAbbrev.selectedIndex;
							
	if (intSelectedStateIndex == 0) {
		myForm.Country.selectedIndex = 0;
	}
	else if ((intSelectedStateIndex >= 1) &&  (intSelectedStateIndex <= 51)) {
		myForm.Country.selectedIndex = findCountry('United States');
	}
	else if (intSelectedStateIndex >= 52 ) {
		myForm.Country.selectedIndex = findCountry('Canada');
	} 
	else {
		myForm.Country.selectedIndex = 0;
	}
...
function changeCountryPullDown() {
	var myForm = document.forms["frmName"];
	var intSelectedStateIndex = myForm.StateAbbrev.selectedIndex;
	if ((intSelectedStateIndex) >= 1 &&  (intSelectedStateIndex <= 51)) {
			myForm.Country.selectedIndex = findCountry('United States');
	}
	else if (intSelectedStateIndex >= 52) {
	...
	else if (intSelectedStateIndex = 0){
		myForm.Country.selectedIndex = 0;
	}
}
...
function findCountry(strCountryName) {
	var myFormElement = document.forms["frmName"].Country;
	var intNumOfOptions = myFormElement.length;
	for (var i = 0; i < intNumOfOptions; i++) {
		if (myFormElement.options[i].text == strCountryName) {
			return i;
		}
	}
}

        ...
        try {
            if (!Page.IsPostBack) {
                ...
                if(itemNumbers != null) {
                    ...
                    //Check DUNS# (xxx000000000xxxxxxxxxx)
                    //- go to custom site if DUNS# indicates AND not from this sites default page
                    bool fromDefault = (bool)Session["FromDefault"];
                    string bnduns = (string)Application["BNDUNS"];
                    string bzduns = (string)Application["BOOKAZINEDUNS"];
                    if (!fromDefault && numbers[0].Length == 22 && numbers[0].Substring(4,9) == bnduns) {
                        Response.Redirect(Application["BNTracker"] + "?id=" + itemNumbers,false);
                    }
                    else if (!fromDefault && numbers[0].Length == 22 && numbers[0].Substring(4,9) == bzduns) {
                        Response.Redirect(Application["BNTracker"] + "?id=" + itemNumbers,false);
                    }
                    else if (!fromDefault && numbers[0].Length == 30 && numbers[0].Substring(12,9) == bnduns) {
                        Response.Redirect(Application["BNTracker"] + "?id=" + itemNumbers,false);
                    }
                    else if (!fromDefault && numbers[0].Length == 30 && numbers[0].Substring(12,9) == bzduns) {
                        Response.Redirect(Application["BNTracker"] + "?id=" + itemNumbers,false);
                    }
                    else {
                        track(numbers);

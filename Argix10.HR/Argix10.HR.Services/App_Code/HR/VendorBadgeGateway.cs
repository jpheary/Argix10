...
namespace Argix.HR {
	//
    public class VendorBadgeGateway {
        //Members
        public const string SQL_CONNID = "Vendors";
        ...
        public DataSet ReadBadge(int idNumber) {
            ...
            DataSet badges = null;
            try {
                badges = new DataService().FillDataset(SQL_CONNID,USP_BADGE,TBL_BADGES,new object[] { idNumber });
            }
            catch (Exception ex) { throw new ApplicationException(ex.Message,ex); }
            return badges;
        }

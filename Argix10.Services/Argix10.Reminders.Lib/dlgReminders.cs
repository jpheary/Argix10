...
namespace Argix {
    ...
    public partial class dlgReminders:Form {
        ...
        //Interface
        public dlgReminders(ReminderService service, RemindersDataset reminders) {
            //Constructor
            InitializeComponent();
                ...
                switch(btn.Name) {
                    case "btnSnooze":
                        if(this.dgvReminders.SelectedRows.Count > 0) {
                            id = Convert.ToInt32(this.dgvReminders.SelectedRows[0].Cells["IssueID"].Value);
                            userID = this.dgvReminders.SelectedRows[0].Cells["UserID"].Value.ToString();
                            time = DateTime.Now;    //.Parse(this.dgvReminders.SelectedRows[0].Cells["Time"].Value.ToString());
                    ...
                    case "btnOpenItem":
                        if(this.dgvReminders.SelectedRows.Count > 0) {
                            id = Convert.ToInt32(this.dgvReminders.SelectedRows[0].Cells["IssueID"].Value);
                            message = this.dgvReminders.SelectedRows[0].Cells["Message"].Value.ToString();
